...
array_strings_t* array_strings_create() {
  ...
  if (array) {
    ...
    array->strings = (char**) calloc(1, sizeof(char**));
    if (array->strings) {
      // Allocate one string
      array->strings[0] = (char*) calloc(100, sizeof(char));
      if (array->strings[0]) {
        return array;
      } else {
        free(array->strings);
        free(array);
        fprintf(stderr, "Error: Couldn't allocate first string\n");
        return NULL;
      }
    } else {
      free(array);
