...
/**
 * @brief Data to generate a bunch of passwords.
 * @details Struct that gives information to threads so they can generate
 * passwords of 'password_length' characters starting at 'start' and finishing
 * at 'finish'.
*/
typedef struct password_block {
  uint64_t start;
  uint64_t finish;
  int64_t password_length;
} password_block_t;

/**
 * @brief A node for this queue. It contains a block and a pointer to the next
 ...
 * @param queue The queue
 * @param block The block that will be enqueued
 ...
 * @param queue The queue
 * @param block The block in which the queue's head will be returned.
 * @returns A status code. 0 means success
*/
