...
array_strings_t* create_copies(char* source_path, uint64_t count) {
  ...
  array_strings_t *copies = array_strings_create();
  if (copies) {
...
void remove_files(array_strings_t* files) {
  uint64_t count = array_strings_get_count(files);
  char path[200];
  for (uint64_t i = 0; i < count; ++i) {
    array_strings_get_string(files, i, path);
    // printf("Remove %s\n", path);
...
array_strings_t* create_files(uint64_t count) {
  array_strings_t* files = array_strings_create();
